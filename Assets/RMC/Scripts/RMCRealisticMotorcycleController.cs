...
public class RMCRealisticMotorcycleController : MonoBehaviour {
	...
	void Start (){
		...
		// SoundsInitialize(); // now done when biker jumps on !
		if(WheelSlipPrefab)
			SmokeInit();
	...
	void OnTriggerEnter (Collider coll){
		...
		if (coll.tag == "Player"){
			...
			SoundsInitialize();
			coll.SendMessage("ActivateRagdoll");
		}
	...
	void OnCollisionEnter (Collision collision){
		if (collision.collider.transform.IsChildOf (transform)) {
			//Debug.Log ("biker collided bike");
		} else {
			if (collision.contacts.Length > 0) {
				// to do better manage magnitude and ragdoll jumping crash
